...
This script simulates BP+OSD decoding for an open tile CSS code under a finite-bias
Pauli noise model. The code is constructed once for a given lattice size (l, m),
and the logical error rate is evaluated over a sweep of physical error rates.

The simulation supports:
    - CSS-code construction from open tile-code checks
    - joint binary decoding through a 2n-variable parity-check representation
    - finite-bias Pauli noise with bias [pX : pY : pZ] = [1 : 1 : eta]
    - optional deformed error channels
    - parallel execution over independent chunks of Monte Carlo shots

Usage
-----
python performance.py <l> <m> --bias <eta>

Example
-------
python performance.py 12 12 --bias 100
"""
...
class css_decode_sim:
    ...
    ms_scaling_factor : float
        The minimum-sum scaling factor.
    max_iter : int
    ...
    run_sim : bool
        If enabled, the simulation starts automatically.
    hadamard_rotate : bool
        Toggle Hadamard rotate. ON: 1; OFF: 0
    hadamard_rotate_sector1_length : int
        Number of qubits in sector 1 for the Hadamard rotation.
    ...
    apply_deformed_error : bool
        Whether to apply the deformed error channel.
    """
    ...
    def _construct_code(self):
        """
        Install hx, hz, lx, lz with no validation.
        """
    ...
    def _error_channel_setup(self):
        """
        Set up the error channels from the error rate and error bias.
        """
    ...
    def _decoder_setup(self):
        """
        Joint Pauli decoding on the 2n-variable binary system:
...
def create_tile_code(l, m, B=3):
    """
    Create an open tile code with given parameters l, m, and B.
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Tile code finite-bias code-capacity study for one (l, m, bias) combination"
    )
...
def main():
    ...
    csv_path = os.path.join(
        OUTPUT_ROOT,
        f"tile_open_css_deformed_parity_final_non_css_independent_l{l}_m{m}_eta{int(eta)}.csv",
    )
