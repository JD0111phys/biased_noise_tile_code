# TI (0.25, 0.5) tile code - Z memory
...
def finish_surface_code_circuit(
    ...
    Hadamard_qubits = [p2q[q] for q in custom_coords]
    ...
    cyx_targets: List[List[int]] = [[], [], [], [],[],[]]
    cyz_targets: List[List[int]] = [[], [], [], [],[],[]]

    for k in range(6):
    # Handle X-checks (x_measure_coords)
...
#                 elif k in [1, 3, 5]:
#                     cyx_targets[k].extend([ctrl, target])
                else:
                    cnotx_targets[k].extend([ctrl, target])


        # Handle Z-checks (z_measure_coords)
    ...
    #print(f"Step {k} — x: ctrl={ctrl}, target={target}, coord={data}")
   ...
   #params.append_unitary_1(cycle_actions, "H", x_measurement_qubits)
    ...
    # Middle layers: x[k] together with z[k-1]
    for k in range(1, 6):
        if czx_targets[k]:
        ...
        if cyz_targets[k - 1]:
            params.append_unitary_2(cycle_actions, "CY", cyz_targets[k - 1])

        cycle_actions.append_operation("TICK", [])

    # Final layer: last z-stream at k = 5
    if cnotz_targets[5]:
    ...
    if cyz_targets[5]:
        params.append_unitary_2(cycle_actions, "CY", cyz_targets[5])
    ...
    #params.append_reset(head, vertical_qubits, "Z")
    ...
    #params.append_unitary_3(head,"S", vertical_qubits)
    ...
    #params.append_unitary_3(tail,"S_DAG", vertical_qubits)
    tail.append_operation("TICK",[])
    #params.append_measure(tail, vertical_qubits, "Z")
...
#     hx_ancilla_coords = {i: c.real - 0.5  + 1j * c.imag for i, c in hhx_ancilla_coords.items()}
    ...
    def generate_custom_coords(m: int) -> set:
        coords = set()
        ...
        return coords
    custom_coords = generate_custom_coords(m)
    #print(custom_coords)
...
def main():
    ...
    filename = f"parallel_special_TILE_middle_Z_memory_logical_error_round8_open_rates_l{l}_m{m}_bias{int(bias)}.csv"
